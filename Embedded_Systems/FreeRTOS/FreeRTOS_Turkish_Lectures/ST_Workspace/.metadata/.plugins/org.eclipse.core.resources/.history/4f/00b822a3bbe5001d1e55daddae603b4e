...
static void prvBacklightTimerCallback(TimerHandle_t xTimer)
{
	...
	sprintf(buffer,"Timer expired, backlight OFF at time %ld\r\n", xTimeNow);
	HAL_UART_Transmit(&huart2, (uint8_t *)buffer, 60, HAL_MAX_DELAY);
}
static void vKeyHitTask(void *pvParameters)
{
	const TickType_t xShortDelay = pdMS_TO_TICKS(50);
	BaseType_t xKeyPressesStopApplication;
	TickType_t xTimeNow;
	char buffer[50];

	xKeyPressesStopApplication = pdFALSE;
	HAL_UART_Transmit(&huart2, (uint8_t *)"Press a key to turn the backlight on.\r\n", 60, HAL_MAX_DELAY);
	...
	for(;;)
	{
		...
		if(HAL_GPIO_ReadPin(GPIOA, GPIO_PIN_0) != GPIO_PIN_RESET)
		{
			...
			if(xSimulatedBacklightOn == pdFALSE)
			{
				...
				sprintf(buffer,"Key pressed, turning backlight ON at time %ld", xTimeNow);
				HAL_UART_Transmit(&huart2,(uint8_t *)buffer, 60,HAL_MAX_DELAY);
			}
			else
			{
				sprintf(buffer,"Key pressed, reseting software timer at \t %ld", xTimeNow);
				HAL_UART_Transmit(&huart2, (uint8_t *)buffer, 60, HAL_MAX_DELAY);
			}
